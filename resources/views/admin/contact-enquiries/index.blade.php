...
<div class="admin-page-head">
    <div>
        ...
        <p class="admin-page-subtitle">Contact page appointment enquiry submissions</p>
    </div>
</div>
...
<div class="stats-grid">
    ...
    <div class="stat-card">
        ...
        <p class="stat-value">{{ $contactEnquiries->whereNotNull('preferred_date')->count() }}</p>
    </div>
    <div class="stat-card">
        <p class="stat-label">With Service</p>
        <p class="stat-value">{{ $contactEnquiries->whereNotNull('service_id')->count() }}</p>
    </div>
</div>
...
<div class="page-card">
    <div class="page-card-header">
        ...
        <span class="page-card-note">
            ...
            These are stored separately from appointment page enquiries
        </span>
    </div>
    ...
    <div class="page-card-table">
        <table class="min-w-full datatable datatable-ContactEnquiry">
            ...
            <tbody>
                @foreach($contactEnquiries as $enquiry)
                        ...
                        <td>
                            <div class="action-row">
                                @can('contact_enquiry_show')
                                    <a href="{{ route('admin.contact-enquiries.show', $enquiry->id) }}" class="btn-outline btn-outline-edit">
                                        <i class="fas fa-eye"></i>
                                        View
                                    </a>
                                @endcan
                                @can('contact_enquiry_delete')
                                    <form action="{{ route('admin.contact-enquiries.destroy', $enquiry->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('{{ trans('global.areYouSure') }}')">
                                        @method('DELETE')
                                        @csrf
